...
class ScenarioResult:
    ...
    def to_junit(self) -> str:
        ...
        duration = self.total_duration_ms / 1000
        lines = [
            '<?xml version="1.0" encoding="UTF-8"?>',
            f'<testsuite name="{self.name}" tests="{len(self.steps)}" '
        ...
        for step in self.steps:
            if step.success:
                lines.append(
                    f'  <testcase name="{step.name}" time="{step.duration_ms / 1000:.3f}"/>'
                )
            else:
                lines.append(
                    f'  <testcase name="{step.name}" time="{step.duration_ms / 1000:.3f}">'
...
class ScenarioRunner:
        ...
        try:
                    ...
                    if "condition" in wait_def:
                        ...
                        if wait_result.get("timeout"):
                            raise AssertionError(
                                f"Wait condition timed out: {wait_def['condition']}"
                            )
            ...
            if "assert" in step:
                ...
                if "player" in current_state:
                    ...
                    prev_pos = (
                        self._previous_state.get("player", {}).get("position", {})
                        if self._previous_state
                        else {}
                    )
                    print(
                        f"  [DEBUG] current pos: ({pos.get('x')}, {pos.get('y')}), "
                        f"prev pos: ({prev_pos.get('x')}, {prev_pos.get('y')})"
                    )
                else:
                    print(f"  [DEBUG] No player in state. Keys: {list(current_state.keys())}")
    ...
    def _evaluate_assertion(self, assertion: str, state: dict[str, Any]) -> bool:
        ...
        if " > " in assertion:
            ...
            left_val = self._get_value(left.strip(), state)
            right_val = (
                self._get_value(right.strip(), state) if "$" in right else float(right.strip())
            )
            return left_val > right_val
